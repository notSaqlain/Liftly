      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-1 no-scrollbar">
        ...
        {groupedMessages.map((msg) => {
          ...
          return (
            <div
              key={msg.id}
              className={`flex items-end gap-2 ${mine ? 'flex-row-reverse' : 'flex-row'} ${msg.isFirst ? 'mt-4' : 'mt-0.5'}`}
            >
              ...
              <div className={`max-w-[72%] flex flex-col ${mine ? 'items-end' : 'items-start'}`}>
                ...
                {!mine && msg.isFirst && (
                  <span className="text-[10px] font-bold text-slate-400 mb-1 ml-1">{msg.displayName}</span>
                )}
                ...
                } ${msg.isFirst && !mine ? 'rounded-tl-md' : ''} ${msg.isFirst && mine ? 'rounded-tr-md' : ''}`}>
                  {msg.text}
                </div>

                {/* Time (last in group only) */}
                {msg.isLast && (
                  <span className={`text-[9px] font-semibold text-slate-300 mt-0.5 mx-1`}>
