      ...
      <div className="p-4 space-y-4">
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-white/50 font-bold">No active challenges yet</p>
            <p className="text-white/30 text-sm mt-1">Your gym admin will create them soon.</p>
          </div>
        ) : (
          challenges.map(challenge => {
            ...
            return (
              <div key={challenge.id} className="bg-[#0D1526] rounded-3xl border border-white/5 overflow-hidden">
                <div 
                  className="p-5 cursor-pointer"
                  onClick={() => setExpandedId(isExpanded ? null : challenge.id)}
                >
                  <div className="flex justify-between items-start mb-2">
                    <h3 className="font-black text-white text-lg">{challenge.title}</h3>
                    <div className="flex items-center gap-1 bg-orange-500/15 text-orange-400 border border-orange-500/20 px-2 py-1 rounded-lg text-[10px] font-black uppercase">
                      <Clock size={12} /> {daysLeft}d left
                    </div>
                  </div>
                  <p className="text-white/50 text-sm line-clamp-2">{challenge.description}</p>
                </div>

                {isExpanded && (
                  <div className="px-5 pb-5 border-t border-white/5 pt-4">
                    <h4 className="text-xs font-black text-white/40 uppercase tracking-widest mb-3 flex items-center gap-2">
                      <Target size={14} className="text-liftly-teal" /> Leaderboard
                    </h4>
                    
                    {challenge.entries.length === 0 ? (
                      <p className="text-white/30 text-sm italic mb-4">No entries yet. Be the first!</p>
                    ) : (
                      <div className="space-y-2 mb-4">
                        {challenge.entries.map((entry, idx) => (
                          <div key={entry.id} className="flex items-center justify-between bg-white/5 border border-white/5 p-2.5 rounded-2xl">
                            <div className="flex items-center gap-3">
                              <span className={`w-6 h-6 rounded-lg flex items-center justify-center text-xs font-black ${idx === 0 ? 'bg-yellow-500/20 text-yellow-400' : 'bg-white/10 text-white/50'}`}>
                                #{idx + 1}
                              </span>
                              <div className="flex items-center gap-2">
                                {entry.photoURL ? (
                                  ...
                                  <div className="w-6 h-6 rounded-full bg-white/10 flex items-center justify-center text-[10px] font-black text-white/60">{entry.displayName.charAt(0)}</div>
                                )}
                                <span className="font-bold text-sm text-white/80">{entry.displayName}</span>
                              </div>
                            </div>
                            <span className="font-black text-liftly-teal">{entry.value} <span className="text-[10px] text-white/30 uppercase">{challenge.metric}</span></span>
                          </div>
                        ))}
                      </div>
                    )}
                    ...
                    <button 
                      ...
                      className="w-full py-3 bg-liftly-teal/10 hover:bg-liftly-teal/20 border border-liftly-teal/20 text-liftly-teal font-black rounded-2xl flex items-center justify-center gap-2 transition-all"
                    >
            ...
            <form onSubmit={handleLogProgress}>
              <div className="flex items-center gap-3 mb-6">
                <input
                  ...
                  className="flex-1 bg-white/5 border border-white/10 rounded-2xl px-4 py-3 font-bold text-lg text-white placeholder:text-white/30 focus:outline-none focus:border-liftly-teal/50 focus:ring-1 focus:ring-liftly-teal/20"
                  placeholder="0"
                  required
                />
                <span className="font-black text-white/40 uppercase tracking-widest">{logModal.metric}</span>
              </div>
              
              <div className="flex gap-2">
                <button type="submit" className="flex-1 py-3.5 bg-liftly-teal text-liftly-navy font-black rounded-2xl shadow-teal active:scale-95 transition-all">Save</button>
                <button type="button" onClick={() => setLogModal(null)} className="flex-1 py-3.5 bg-white/10 text-white/60 font-black rounded-2xl hover:bg-white/15 transition-all">Cancel</button>
              </div>
            </form>
          </div>
        </div>
