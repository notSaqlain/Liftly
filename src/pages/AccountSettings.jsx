  ...
  return (
    <div className="bg-slate-50 min-h-full flex flex-col animate-fade-in">
      ...
      <div className="bg-liftly-navy px-5 pt-12 pb-6 relative overflow-hidden shrink-0">
        <div className="absolute top-0 right-0 w-32 h-32 bg-slate-500/20 rounded-full blur-3xl -mr-10 -mt-10" />
      ...
      <div className="flex-1 overflow-y-auto p-5 pb-24 space-y-4">
        ...
        <div className="flex gap-2">
          {googleLinked && (
            <span className="px-3 py-1.5 bg-blue-50 text-blue-600 text-[10px] font-black rounded-xl uppercase tracking-wider flex items-center gap-1.5 border border-blue-100">
          ...
          {passwordLinked && (
            <span className="px-3 py-1.5 bg-slate-100 text-slate-600 text-[10px] font-black rounded-xl uppercase tracking-wider border border-slate-200">Email/Password</span>
          )}
        </div>
        ...
        <AccordionItem 
          ...
          icon={<Ghost size={18} />} 
          iconBg={hideOnlineStatus ? "bg-slate-800 text-white" : "bg-slate-100 text-slate-500"} 
          isOpen={activeSection === 'privacy'} 
          onToggle={() => setActiveSection(activeSection === 'privacy' ? null : 'privacy')}
        >
          <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 mb-4">
            <div className="flex items-center justify-between mb-2">
              <span className="font-black text-slate-800 text-sm">Incognito Mode</span>
              <button 
                onClick={() => setHideOnlineStatus(!hideOnlineStatus)}
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${hideOnlineStatus ? 'bg-liftly-teal' : 'bg-slate-300'}`}
              >
        ...
        {googleLinked && !passwordLinked ? (
          <AccordionItem title="Backup Email" subtitle="Add a recovery email" icon={<Mail size={18} />} iconBg="bg-emerald-50 text-emerald-500" isOpen={activeSection === 'email'} onToggle={() => setActiveSection(activeSection === 'email' ? null : 'email')}>
            ...
            <input type="email" placeholder="Backup email address" className="w-full h-12 bg-slate-50 border border-slate-200 rounded-xl px-4 text-sm text-slate-800 font-semibold focus:outline-none focus:border-liftly-teal focus:ring-1 focus:ring-liftly-teal mb-4 transition-all" value={backupEmail} onChange={(e) => setBackupEmail(e.target.value)} />
          ...
          <AccordionItem title="Email Address" subtitle={currentUser?.email} icon={<Mail size={18} />} iconBg="bg-emerald-50 text-emerald-500" isOpen={activeSection === 'email'} onToggle={() => setActiveSection(activeSection === 'email' ? null : 'email')}>
            <input type="email" placeholder="New email address" className="w-full h-12 bg-slate-50 border border-slate-200 rounded-xl px-4 text-sm text-slate-800 font-semibold focus:outline-none focus:border-liftly-teal focus:ring-1 focus:ring-liftly-teal mb-4 transition-all" value={newEmail} onChange={(e) => setNewEmail(e.target.value)} />
        ...
        {passwordLinked && (
          <AccordionItem title="Change Password" subtitle="Update your password" icon={<Lock size={18} />} iconBg="bg-purple-50 text-purple-500" isOpen={activeSection === 'password'} onToggle={() => setActiveSection(activeSection === 'password' ? null : 'password')}>
            <div className="space-y-3 mb-4">
              <input type="password" placeholder="New password" className="w-full h-12 bg-slate-50 border border-slate-200 rounded-xl px-4 text-sm text-slate-800 font-semibold focus:outline-none focus:border-liftly-teal focus:ring-1 focus:ring-liftly-teal transition-all" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} />
              <input type="password" placeholder="Confirm new password" className="w-full h-12 bg-slate-50 border border-slate-200 rounded-xl px-4 text-sm text-slate-800 font-semibold focus:outline-none focus:border-liftly-teal focus:ring-1 focus:ring-liftly-teal transition-all" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} />
            </div>
        ...
        {passwordLinked && !googleLinked && (
          ...
          >
            ...
            <button onClick={() => setShowGoogleWarning(false)} className="w-full h-11 bg-slate-100 text-slate-500 hover:text-slate-600 hover:bg-slate-200 font-black text-sm rounded-xl flex items-center justify-center transition-all active:scale-95">
              Cancel
            </button>
          </AccordionItem>
        )}
        ...
        >
          ...
          <button onClick={() => setShowDeleteConfirm(false)} className="w-full h-11 bg-slate-100 text-slate-500 hover:text-slate-600 hover:bg-slate-200 font-black text-sm rounded-xl flex items-center justify-center transition-all active:scale-95">
            Cancel
          </button>
        </AccordionItem>
        ...
        {/* ─── Logout ─── */}
        <div className="pt-4">
          <button onClick={handleLogout} className="w-full h-14 bg-white hover:bg-red-50 text-red-500 font-black rounded-2xl flex items-center justify-center space-x-2 transition-all active:scale-95 border border-red-100 shadow-sm">
...
const AccordionItem = ({ title, subtitle, icon, iconBg, isOpen, onToggle, children }) => (
  <div className="mb-3">
    <button onClick={onToggle} className={`w-full bg-white rounded-2xl p-4 flex items-center justify-between shadow-card border border-slate-100 active:scale-[0.98] transition-all ${isOpen ? 'rounded-b-none border-b-0' : ''}`}>
      <div className="flex items-center gap-3">
        ...
        <div className="text-left">
          ...
          <p className="text-slate-400 text-xs font-medium truncate max-w-[200px]">{subtitle}</p>
        </div>
      </div>
      <ChevronRight size={18} className={`text-slate-300 transition-transform duration-200 ${isOpen ? 'rotate-90' : ''}`} />
    </button>
    {isOpen && (
      <div className="bg-white rounded-b-2xl p-4 pt-2 shadow-card border border-t-0 border-slate-100 animate-fade-in">
